...
public class IniResultFetcher implements IResultFetcher{
	...
	public String fetch(JobExecutionContext context)
		...
		root.addAttribute("start_time", start_time_str == null || start_time_str.isEmpty() ? new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date()) : start_time_str);
		root.addAttribute("end_time", end_time_str == null || end_time_str.isEmpty() ? new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date()) : end_time_str);
