...
public class TaskFetch {
    ...
    public TaskFetch(ConfigurationFile config, TaskSchedule ts, long sync_task_interval_time) {
        this.ts = ts;
        this.config = config;
        this.task_sync = new TaskSyncRunable(config, this.ts);
    ...
    public void fetch() throws Exception {
        String url = config.getValue(null, "aitask.sync.url");
        String agent_name = config.getValue(null, "aitask.name");
        ...
        worker.Post(EntityUtils.toString(new UrlEncodedFormEntity(pairs, "UTF-8")), ContentType.APPLICATION_JSON);
