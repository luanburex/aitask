...
public class IniResultFetcherTester {
	...
	public void testNormal() throws JobExecutionException, DocumentException{
		...
		String ini_result_file = Thread.currentThread().getContextClassLoader().getResource("").getPath().toString() 
		...
		IniResultFetcher fetcher = new IniResultFetcher();
		...
		Assert.assertEquals("1", root.element("steps").element("ResultStep").attributeValue("step_id"));
		Assert.assertEquals("登录", root.element("steps").element("ResultStep").attributeValue("step_name"));
		Assert.assertEquals("0", root.element("steps").element("ResultStep").attributeValue("result"));
		Assert.assertEquals("3549", root.element("steps").element("ResultStep").attributeValue("eclapse"));
